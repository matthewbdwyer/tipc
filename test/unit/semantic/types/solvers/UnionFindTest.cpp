...
static std::vector<std::shared_ptr<TipType>>
intsToTipVars(std::vector<int> &values) {
...
TEST_CASE("UnionFind: Test find", "[UnionFind]") {
  ...
  auto actualRootOfFour =
      std::dynamic_pointer_cast<TipVar>(unionFind.find(four));
  auto actualIntOfRootOfFour =
      dynamic_cast<ASTNumberExpr *>(actualRootOfFour->getNode())->getValue();
  auto actualRootOfNine =
      std::dynamic_pointer_cast<TipVar>(unionFind.find(nine));
  auto actualIntOfRootOfNine =
      dynamic_cast<ASTNumberExpr *>(actualRootOfNine->getNode())->getValue();
