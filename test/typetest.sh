...
../build/tipc $1
cd ~/tipc/build/
./tipc -t ../test/$1 > /tmp/$USER/$bname.tipc-type-out
# replace alpha type id since they are different in tipc and tipscala
sed -i 's/α<.*>/α<>/g' /tmp/$USER/$bname.tipc-type-out
...
sed -i 's/\[\(0\|1\|31\)m//g' /tmp/$USER/$bname.tipscala-type-out
# replace alpha type id since they are different in tipc and tipscala
sed -i 's/α<.*>/α<>/g' /tmp/$USER/$bname.tipscala-type-out
