 ...
 * edges, children to parents. The output adheres to the dot syntax and can be
 * converted to a format suitable for visualization.
 *
 * For example, if you run the visualizer and write its output to a file called
 * foo.gv, you can build a png from it using the dot command line interface.
 * \code{.sh}
