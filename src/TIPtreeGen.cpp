...
static llvm::Function *getFunction(std::string Name) {
  ...
  if (Name == "main") {
      ...
      // Declare "_tip_main()"
      auto *M = llvm::Function::Create(FunctionType::get(Type::getInt64Ty(TheContext), 
                                                         false),
      ...
      auto *FT = FunctionType::get(Type::getInt64Ty(TheContext), 
                                   FormalTypes, 
                                   false);
...
// REFACTOR: why can't we use the IRBuilder for the pass?
...
std::unique_ptr<llvm::Module> Program::codegen(std::string programName) {
  ...
  { 
     ...
     auto *genFunPtrType = PointerType::get(
                               FunctionType::get(
                                   Type::getInt64Ty(TheContext), None, false),
                               0);
     ...
     for (auto const& pf : programFunctions) {
         castProgramFunctions.push_back(ConstantExpr::getPointerCast(pf, genFunPtrType));
     };     
     ...
     tipFTable = new GlobalVariable(*CurrentModule,
                                    ftableType,
                                    true,
  ...
  { 
      ...
      if (fd == FunctionDecls.end()) {
         auto *M = llvm::Function::Create(FunctionType::get(
                                              Type::getInt64Ty(TheContext), 
                                              false),
                                          llvm::Function::ExternalLinkage, 
                                          "_tip_main",
         ...
         auto *undef = llvm::Function::Create(FunctionType::get(
      ...
      tipNumInputs = new GlobalVariable(*CurrentModule,
                                        Type::getInt64Ty(TheContext),
                                        true,
      ...
      std::vector<Constant *> zeros(numTIPArgs, 
                                    ConstantInt::get(Type::getInt64Ty(TheContext), 0));
      tipInputArray = new GlobalVariable(*CurrentModule,
                                         inputArrayType,
                                         false,
                                         llvm::GlobalValue::CommonLinkage,
                                         ConstantArray::get(inputArrayType, zeros),
                                         "_tip_input_array");
  }
...
llvm::Function *Function::codegen() {
  ...
  for (auto const& decl : DECLS) {
    if (!decl->codegen()) success = false;
  }

  for (auto &stmt : BODY) {
    if (!stmt->codegen()) success = false;
  }
...
llvm::Value *InputExpr::codegen() {
  if (inputIntrinsic == nullptr) {
      ...
      inputIntrinsic = llvm::Function::Create(FT, 
                                              llvm::Function::ExternalLinkage, 
                                              "_tip_input", 
                                              CurrentModule.get());
  }
...
llvm::Value *FunAppExpr::codegen() {
  ...
  auto *funPtrType = PointerType::get(FunctionType::get(Type::getInt64Ty(TheContext), 
                                                        actualTypes, 
                                                        false),
                                      0);
...
llvm::Value *AllocExpr::codegen() {
  ...
  if (mallocFun == nullptr) {
      ...
      auto *FT = FunctionType::get(Type::getInt8PtrTy(TheContext), 
                                   oneInt, 
                                   false);
      mallocFun = llvm::Function::Create(FT, 
                                         llvm::Function::ExternalLinkage, 
                                         "malloc", 
                                         CurrentModule.get());
...
llvm::Value *NullExpr::codegen() {
  ...
  return Builder.CreatePtrToInt(nullPtr, 
                                Type::getInt64Ty(TheContext),
                                "nullPtrIntVal");
}
...
llvm::Value *RefExpr::codegen() {
  ...
  return Builder.CreatePtrToInt(argVal, 
                                Type::getInt64Ty(TheContext),
                                "intPtrVal");
}
...
llvm::Value *DeRefExpr::codegen() {
  if (lValueGen) {
    ...
    return Builder.CreateIntToPtr(argVal, 
                                  Type::getInt64PtrTy(TheContext),
                                  "ptrIntVal");
    ...
    auto *ref = Builder.CreateIntToPtr(argVal, 
                                       Type::getInt64PtrTy(TheContext),
                                       "ptrIntVal");
...
llvm::Value *DeclStmt::codegen() {
  ...
  for (auto l : VARS) {
    ...
    Builder.CreateStore(
       ConstantInt::get(Type::getInt64Ty(TheContext), 0), localAlloca);
...
llvm::Value *WhileStmt::codegen() {
  ...
  BasicBlock *HeaderBB = BasicBlock::Create(TheContext, 
                                            "header"+std::to_string(labelNum),
                                            TheFunction);
  BasicBlock *BodyBB = BasicBlock::Create(TheContext,
                                          "body"+std::to_string(labelNum));
  BasicBlock *ExitBB = BasicBlock::Create(TheContext,
                                          "exit"+std::to_string(labelNum));
...
llvm::Value *IfStmt::codegen() {
   ...
   */
  labelNum++; // create unique labels for these BBs
  BasicBlock *ThenBB = BasicBlock::Create(TheContext, 
                                          "then"+std::to_string(labelNum), TheFunction);
  BasicBlock *ElseBB = BasicBlock::Create(TheContext, 
                                          "else"+std::to_string(labelNum));
  BasicBlock *MergeBB = BasicBlock::Create(TheContext, 
                                           "ifmerge"+std::to_string(labelNum));
...
llvm::Value *OutputStmt::codegen() {
  if (outputIntrinsic == nullptr) {
      ...
      outputIntrinsic = llvm::Function::Create(FT, 
                                               llvm::Function::ExternalLinkage, 
                                               "_tip_output", 
                                               CurrentModule.get());
  }
...
llvm::Value *ErrorStmt::codegen() {
  if (errorIntrinsic == nullptr) {
      std::vector<Type*> oneInt(1, Type::getInt64Ty(TheContext));
      auto *FT = FunctionType::get(Type::getInt64Ty(TheContext), oneInt, false);
      errorIntrinsic = llvm::Function::Create(FT, 
                                              llvm::Function::ExternalLinkage, 
                                              "_tip_error", 
                                              CurrentModule.get());
  }
