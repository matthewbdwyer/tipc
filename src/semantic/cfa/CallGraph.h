...
/*! \class CallGraph
 *  \brief Performs control flow analysis and records results for subsequent
 * phases Generates call graph of a program that represents calling
 * relationships between subroutines in a computer program. Each node represents
 * a subroutine and each edge (a1, a0) indicates that procedure a1 calls
 * procedure a0. A cycle in the graph indicates recursive procedure calls, e.g.,
 * a0 -> a0 indicates that a0 calls itself recursively this call graph is
 * sometimes approximations. Not all the call relationship that exist in the
 * graph will occur in the actual runs of the program.
 */
...
public:
  ...
  /*! \brief Return the set of functions that may be called at an application
   * expr
   */
   ...
   * name of a function \return The set of all callee functions node
   */
  ...
  /*! \brief Returns bool value indicating whether there is an edge between two
   * subroutines. \param caller and callee are the str name of the subroutines
