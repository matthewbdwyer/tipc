...
ASTProgram::codegen(SemanticAnalysis *semanticAnalysis,
   ...
   */
  {
    ...
    // -Saket : Trying to make this opaque pointer

    auto *genFunOpaquePtrType = llvm::PointerType::get(llvmContext, 0);
    auto *oldGenFunType =
        llvm::FunctionType::get(llvm::Type::getInt64Ty(llvmContext), false);
    // Create Record Dispatch Table

    auto *functionTableType =
        llvm::ArrayType::get(genFunOpaquePtrType, funIndex);
    ...
    for (auto const &pf : programFunctions) {
      castProgramFunctions.push_back(
          llvm::ConstantExpr::getPointerCast(pf, genFunOpaquePtrType));
     ...
     */
    auto *ftableInit = llvm::ConstantArray::get(functionTableType, castProgramFunctions);
    
    // Create the global function dispatch table
    tipFunctionTable = new llvm::GlobalVariable(*CurrentModule, functionTableType, true,
    ...
    auto *FT = llvm::FunctionType::get(llvm::PointerType::get(llvmContext,0), twoInt, false);
    ...
    globalRecordType = llvm::StructType::create(llvmContext, member_values, "uberRecord");
  ...
  llvm::Value *ASTFunction::codegen() {
    ...
    llvm::BasicBlock *BB = llvm::BasicBlock::Create(llvmContext, "entry", TheFunction);
    irBuilder.SetInsertPoint(BB);
    ...
    if (getName() == "main") {
      ...
      for (auto &argName : functionFormalNames[getName()]) {
        ...
        indices.push_back(llvm::ConstantInt::get(llvm::Type::getInt64Ty(llvmContext), argIdx));
        auto *gep = irBuilder.CreateInBoundsGEP(tipInputArray->getValueType(),
                                              tipInputArray, indices, "inputidx");
  
        // Load the value and store it into the arg's alloca
        //      auto *inVal =
        //          irBuilder.CreateLoad(gep->getType()->getPointerElementType(),
        //          gep,
        //                             "tipinput" + std::to_string(argIdx++));
        auto *inVal =
            irBuilder.CreateLoad(llvm::Type::getInt64Ty(llvmContext),
                               gep, "tipinput" + std::to_string(argIdx++));
  ...
  llvm::Value *ASTNumberExpr::codegen() {
    ...
    return llvm::ConstantInt::get(llvm::Type::getInt64Ty(llvmContext), getValue());
  } // LCOV_EXCL_LINE
  ...
  llvm::Value *ASTVariableExpr::codegen() {
    ...
    return llvm::ConstantInt::get(llvm::Type::getInt64Ty(llvmContext), fidx->second);
  }
  ...
  llvm::Value *ASTInputExpr::codegen() {
    ...
    if (inputIntrinsic == nullptr) {
      auto *FT = llvm::FunctionType::get(llvm::Type::getInt64Ty(llvmContext), false);
  ...
  llvm::Value *ASTFunAppExpr::codegen() {
    ...
    auto *gep = irBuilder.CreateInBoundsGEP(tipFunctionTable->getValueType(), tipFunctionTable,
                                          indices, "ftableidx");
  
    // Load the function pointer
    auto *genericFunPtr = irBuilder.CreateLoad(
    ...
    std::vector<llvm::Type *> actualTypes(getActuals().size(),
                                          llvm::Type::getInt64Ty(llvmContext));
    auto *funType =
        llvm::FunctionType::get(llvm::Type::getInt64Ty(llvmContext), actualTypes, false);
    auto funPtrType = genericFunPtr;
    ...
    return irBuilder.CreateCall(funType, funPtrType, argsV, "calltmp");
  }
  ...
  llvm::Value *ASTAllocExpr::codegen() {
    ...
    twoArg.push_back(llvm::ConstantInt::get(llvm::Type::getInt64Ty(llvmContext), 1));
    twoArg.push_back(llvm::ConstantInt::get(llvm::Type::getInt64Ty(llvmContext), 8));
    auto *allocInst = irBuilder.CreateCall(callocFun, twoArg, "allocPtr");

    // Initialize with argument
    auto *initializingStore = irBuilder.CreateStore(argVal, allocInst);
  
    return irBuilder.CreatePtrToInt(allocInst, llvm::Type::getInt64Ty(llvmContext),
                                  "allocIntVal");
  }
  
  llvm::Value *ASTNullExpr::codegen() {
    auto *nullPtr = llvm::ConstantPointerNull::get(llvm::PointerType::get(llvmContext,0));
  ...
  llvm::Value *ASTDeRefExpr::codegen() {
    ...
    if (isLValue) {
      ...
      return irBuilder.CreateLoad(llvm::Type::getInt64Ty(llvmContext), address, "valueAt");
      //    return irBuilder.CreateLoad(address->getType()->getPointerElementType(),
    }
  }
  ...
  llvm::Value *ASTRecordExpr::codegen() {
    ...
    if (allocFlag) {
      ...
      callocArgs.push_back(
          llvm::ConstantInt::get(llvm::Type::getInt64Ty(llvmContext), sizeOfGlobalRecord));
      ...
      // Load allocaRecord
      auto loadInst = irBuilder.CreateLoad(pointerToGlobalRecordType, allocaRecord);
      ...
      return irBuilder.CreatePtrToInt(recordPtr, llvm::Type::getInt64Ty(llvmContext),
                                    "recordPtr");
    } else {
      ...
      // ints
      return irBuilder.CreatePtrToInt(allocaRecord, llvm::Type::getInt64Ty(llvmContext),
                                    "record");
    }
  }
  ...
  llvm::Value *ASTAccessExpr::codegen() {
    ...
    llvm::Value *recordAddress = irBuilder.CreateIntToPtr(recordVal, pointerToGlobalRecordType);
    ...
    auto *gep =
        irBuilder.CreateStructGEP(globalRecordType, recordAddress, index, currField);
    ...
    auto fieldLoad = irBuilder.CreateLoad(llvm::IntegerType::getInt64Ty(llvmContext), gep);
    return irBuilder.CreatePtrToInt(fieldLoad, llvm::Type::getInt64Ty(llvmContext),
                                  "fieldAccess");
  }
  ...
  llvm::Value *ASTWhileStmt::codegen() {
    ...
    // Emit loop header
    {
      ...
      CondV = irBuilder.CreateICmpNE(CondV, llvm::ConstantInt::get(CondV->getType(), 0),
                                   "loopcond");
  ...
  llvm::Value *ASTIfStmt::codegen() {
    ...
    llvm::BasicBlock *MergeBB =
        llvm::BasicBlock::Create(llvmContext, "ifmerge" + std::to_string(labelNum));
